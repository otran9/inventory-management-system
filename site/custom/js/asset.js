	...
	$("#addAssetModalBtn").unbind('click').bind('click', function() {
		...
		*/
		...
		$("#submitAssetForm").unbind('submit').bind('submit', function() {
			...
			if(assetTagID && assetDescription) {
				...
				$.ajax({
					...
					success:function(response) {
						...
						//NTRAN getData[name] = response; // OTRAN
			...
			// NTRAN - Close Add Asset Modal Form
			//$('#addAssetModal').modal('hide');  //or  $('#addAssetModal').modal('toggle');
			/*
			// NTRAN - Close Add Asset Modal Form - With Timeout
			$(this).delay(3000).hide(10, function() {
				$('#addAssetModal').modal('hide');
...
function editAsset(assetID = null) {
	if(assetID) {
		//alert("ENTERING function editAsset | assetID: " + assetID); //OTRAN
		...
		$.ajax({
			...
			success:function(response) {
			    ...
			    //alert("fetchSelectedAsset.php |  response: " + response); //OTRAN
				...
				$("#editAssetForm").unbind('submit').bind('submit', function() {
					...
					if(assetDescription == "") {
						$("#editAssetDescription").after('<p class="text-danger">Asset Description is required</p>');
					...
					/*
					// OTRAN Auto close the editAssetModal form
					$(this).delay(3000).hide(10, function() {
						$('#editAssetModal').modal('hide');
